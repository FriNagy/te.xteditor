...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInst,LPSTR lpCmdLine,int nCmdShow)
{
  ...
  solang = wcslen(wchWorkingDirectory);
  if (solang > 4) solang = solang - 4;
  if (_wcsicmp(wchWorkingDirectory + solang, L"\\f2p") == 0) {
	  bausf2p = TRUE;

//	  wcscat(wchWorkingDirectory, L"\\..");  // ???? ich starte immer aus f2p, also eine ebene back.
	  wchWorkingDirectory[solang] = '\0';    // ???? ich starte immer aus f2p, also eine ebene back.
  };
 ...
 /* if (!Is2k()) {
    ...
    FormatMessage(
        ...
        0,
        NULL);
	MessageBox(NULL, (LPCWSTR)lpMsgBuf, WC_TU, MB_OK | MB_ICONEXCLAMATION);
    LocalFree(lpMsgBuf);
    return(0);
  } */
  ...
  if ((wcslen(szIniFile)) < 1) {  // ini nicht gesetzt 
	  if (bausf2p) {              // aus f2p gestartet
		  if (myext[1] == '2') {  // setze ini nach Dateiname
			  if ( (myext[0] == 'f')  || (myext[0] == 'F')) lstrcpy(szIniFile, L"f");
			  if ( (myext[0] == 'd')  || (myext[0] == 'D')) lstrcpy(szIniFile, L"d");
		                       };
	                };
  };
...
*/
 ...
 //  hModUxTheme = LoadLibrary(L"uxtheme.dll");  //  = wozu
 //  auch  PrivateIsAppThemed() ge�ndert immer returns FALSE
...
sunieweitr:
 ...
 /* if (flagPasteBoard) {
  ...
  if (flagStartAsTrayIcon)   SetNotifyIconTitle(hwndMain);
...
LRESULT CALLBACK MainWndProc(HWND hwnd,UINT umsg,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(umsg)
  {
    ...
    case WM_CONTEXTMENU:
    {
	  ...
	  if (hmenu = LoadMenu(g_hInstance, MAKEINTRESOURCE(imenu)));
...
/*  EnableCmd(hmenu,IDM_ENCODING_RECODE,i);
 ...
 // EnableCmd(hmenu,IDM_VIEW_CUSTOMIZETB,bShowToolbar);
  ...
  EnableCmd(hmenu, IDM_FILE_PRINTF12, i);
  EnableCmd(hmenu, IDM_FILE_PRINTF11, i);
...
WCHAR *wsrepl(WCHAR *string, WCHAR *oldpiece, WCHAR *newpiece) {
	...
	if ((c = (WCHAR *)wcsstr(string, oldpiece)) == NULL)  {
		...
		newpiece = _wcslwr(_wcsdup(oldpiece));
		c = (WCHAR *)wcsstr(news, newpiece);
	...
	new_len = wcslen(newpiece);
	old_len = wcslen(oldpiece);
	end = wcslen(string) - old_len;
...
int xfilesave(int mypar1)
{
		...
		if (0 == dwRet) wcscpy( tchCmdLine, L".\\" );
			
		dwRet = GetEnvironmentVariable(L"home", lstring, 250);

		if (dwRet) {

			if (lstring[0] != L'\\') {
				if (lstring[1] != L':') {   // relative pfad , also dfroot dazu
					wcscpy(tchCmdLine, lstring);
					wcscpy(lstring, envva);
					wcscat(lstring, L"\\");
					wcscat(lstring, tchCmdLine);
				};
			};

		}
		else {
			SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, lstring);
			wcscat(lstring, L"\\nagy");
		};

		wcscat(lstring, L"\\f2run");
		wcscat(lstring, WC_TU);

		if ((fh = _wfopen(lstring, L"wt")) != NULL)
		{
			...
			// int col1 = (int)SendMessage(hwndEdit, SCI_GETCOLUMN, iPos - 1, 0);


			wsprintf(tchCmdLine, L"|%i.%i (%i)\n", iLn, iCol, mypar1);
			fputws(tchCmdLine, fh);

			iPrevLineLength = (int)SendMessage(hwndEdit, SCI_LINELENGTH, iLn-1 , 0);
			if (pLineBuf = GlobalAlloc(GPTR, iPrevLineLength + 1))
			{
				SendMessage(hwndEdit, SCI_GETLINE, iLn - 1, (LPARAM)pLineBuf);
				*(pLineBuf + iPrevLineLength) = '\0';
				if (*pLineBuf) {

					UINT cp = (UINT)SendMessage(hwndEdit, SCI_GETCODEPAGE, 0, 0);

					int nSize = MultiByteToWideChar(cp, 0, pLineBuf, -1, NULL, 0);  // auf 0 umgestellt in Vers. 87

					if (nSize != 0) {

						WCHAR * pBuffer; //  = new char[nSize + 1];
						pBuffer = GlobalAlloc(GPTR, nSize + 1);
						MultiByteToWideChar(cp, 0, pLineBuf, -1, pBuffer, nSize);


						nUtfSize = WideCharToMultiByte(CP_OEMCP, 0, pBuffer, -1, NULL, 0, NULL, NULL);

						if (nUtfSize != 0)
						{
							WideCharToMultiByte(CP_OEMCP, 0, pBuffer, -1, pLineBuf, nUtfSize, NULL, NULL);
							fputs(pLineBuf, fh);
						}
					};
				GlobalFree(pLineBuf);
			...
			fclose(fh);
			mitf2com = FALSE;
			if ( mypar1 < 0 ) SendMessage(hwndEdit, IDM_FILE_EXIT, 0, 0);
			exit(0);
...
int startmyed(int mymode)
{
 ...
 /*   if (mittemp) {
   ...
   /*     copa old to maktetmp
	...
	FileSave(FALSE, FALSE, FALSE, FALSE);
    ...
    dwRet = GetEnvironmentVariable(L"dfroot", envva, 250);
    if (0 == dwRet) wcscpy( envva, L"C:\\flex");
   //     MsgBox(MBWARN, IDS_ERR_LOADFILE, L"DFROOT ist nicht gesetzt!\nso kann F2P nicht starten");
   //     return(reti);
   //  };


	wcscpy(lstring, envva);
	wcscat(lstring, L"\\f2p\\f2p.exe ");
	wcscat(lstring, szCurFile);
	...
	if (mymode == 2) {   // F2 wurde gedruckt
		...
		wcscat(lstring, WC_TU);
		wcscat(lstring, L".");
		wcscat(lstring, myext);

		lstrcpy(tchCmdLine, szCurFile);
		PathQuoteSpaces(tchCmdLine);

		if ((fh = _wfopen(lstring, L"rt")) == NULL)
		{


			RunDlg(hwndMain, tchCmdLine);
	  	    // gehe zu standard run
			return(0);
		};


		if (fgetws(lstring, 216, fh) == NULL)  {
			fclose(fh);
			return(0);
		};
		...
		if (reti > 3) {
			...
			if ((StrCmpNI(lstring, L"stop", 4)) == 0) {  // stop+c:\dfhome\nagy

				if (lstring[4] == '+')  {

					if ((fh = _wfopen(lstring + 5, L"wt")) != NULL)
					{
						...
						iPos = (int)SendMessage(hwndEdit, SCI_LINEFROMPOSITION, iPos, 0) + 1;
						wsprintf(tchCmdLine, L"|%i", iPos);
						fputws(tchCmdLine, fh);
				...
				SendMessage(hwndEdit, IDM_FILE_EXIT, 0, 0);
				exit(0);
			};
			lstring[reti-1] = L'\0';
		};
		...
		lstrcpy(lstring, (wsrepl(lstring, L"%1", szCurFile)));
		lstrcpy(lstring, (wsrepl(lstring, L".V\\", envva)));


	};


	if (mymode == 11) {   // F11 pdf Ausgabe, Shift-F11 mit Vorschu

		if (GetAsyncKeyState(VK_SHIFT) < 0) {
			wcscat(lstring, L" -yr -m0 -t0");   // rechts ohne menue und toolbox
		}
		else {
			if (GetAsyncKeyState(VK_CONTROL) < 0) {
				reti = 1;
				wcscat(lstring, L" -yl -t3 -v");
			}
			else wcscat(lstring, L" -or -x -a");  // pdf erstellen und anzeigen on Desktop\F2P
		};
                      };
	if (mymode == 12) {   // F11 Druck Ausgabe, Shift-F12 mit Vorschu

		if (GetAsyncKeyState(VK_SHIFT) < 0) {
			wcscat(lstring, L" -yr -m0 -t0 -v");   // rechts ohne menue und toolbox
		}
		else {
			if (GetAsyncKeyState(VK_CONTROL) < 0) {
				reti = 1;
			...
			else wcscat(lstring, L" -odv -t0");  // pdf erstellen und anzeigen
		};
	};

 #ifdef oldoem
	//  wcscat(lstring, L" -k\"dos++ \"");
 #endif
	...
	if (!CreateProcess(NULL, // No module name (use command line). 
		...
		envva,             // start in dfroot 
		&si,              // Pointer to STARTUPINFO structure.
		&pi)             // Pointer to PROCESS_INFORMATION structure.
		)
	{

		MsgBox(MBWARN, IDS_ERR_LOADFILE, L"\nkann F2P nicht starten");
		return(1);
	}

	if (auscline) {

		if (flagSingleFileInstance) waitt = INFINITE;
...
int myRunp(LPCWSTR myCline, WCHAR par2)
{
    ...
    if (par2 > 'a') {
        ...
        wcscpy(lstring, wchWorkingDirectory);
        if (bausf2p) wcscat(lstring, L"\\f2p");
        wcscat(lstring, L"\\tw.exe ");
...
//	_wmakepath(lstring, drive, mdir, L"f2p", L"exe");
        ...
        if (par2 == '0') {   // alles von cmdline
            wcscpy(szCurFile, myCline);
            return(startmyed(-10));
        };

        wcscat(lstring, L"\\f2p\\");
        wcscat(lstring, myCline);
    };
  ...
  switch(LOWORD(wParam))
  {
    ...
    case IDM_FILE_READONLY:
      ...
      if (lstrlen(szCurFile))
      {
        ...
        if (flagbreadonly) bReadOnly = flagbreadonly;
...
/*
	...
	case ID__CHAINF2P:
		...
		if (startmyed(0)) SendMessage(hwnd, WM_CLOSE, 0, 0);

	//	SendMessage(hwndEdit, SCI_UNDO, 0, 0);
		break;

	case IDM_FILE_PRINTF12:
		if (quickoff) {
			xfilesave(-12);
			break;
		};
		if (startmyed(12)) SendMessage(hwnd, WM_CLOSE, 0, 0);
		//	SendMessage(hwnd, WM_CLOSE, 0, 0);

		//	SendMessage(hwndEdit, SCI_UNDO, 0, 0);
		break;
	case IDM_FILE_PRINTF2:
		if (quickoff) {
			xfilesave(-2);
			break;
		};
		if (startmyed(2)) SendMessage(hwnd, WM_CLOSE, 0, 0);
		//	SendMessage(hwnd, WM_CLOSE, 0, 0);
	...
	case IDM_FILE_PRINTF11:
		...
		if (startmyed(11)) SendMessage(hwnd, WM_CLOSE, 0, 0);
		//	SendMessage(hwnd, WM_CLOSE, 0, 0);
...
#ifdef winoem
  iDefaultEncoding = IniSectionGetInt(pIniSection, L"DefaultEncoding", 5);
#endif
#ifdef winutf8
...
//			prc->top = 0;
	...
	if ((tolower(meinch)) == L'r') {
		if (meinch == L'r') xbreit = xbreit / 1.8;
	...
	if ((tolower(meinch)) == L'l') {
		if (meinch == L'l') xbreit = xbreit / 1.8;
		xoben = prc->left;
		return;
...
/*
  ...
  if ((hLibrary = LoadLibrary(L".\\flexddll.dll")) == NULL)

  {
      ErrorMeld(L"Kann library nicht laden.", L"flexddll");


  };
  MYPROC ProcAdd;
  ProcAdd = (MYPROC)GetProcAddress(hLibrary, "myPuts");
*/
  ...
  prot = fopen(".\\BKPROT.LOG", "wb");
   for (i = 0; i < nArgs; i++) {
      wsprintf(myMsg, L"%d: %ws\n", i, szArglist[i]);
      fputws(myMsg, prot);
      fputs("\n\r", prot);
      fputs(szArglist[i], prot);
      fputs("\n\r", prot);
      fputws(szArglist[i], prot);
      fputs("\n\r", prot);
 ...
 // WCHAR tchBuild[MAX_PATH];
  ...
  // ich setz fix die INI File zu %userdir%nagy\datname.ini"
...
//	 if (!parcount && ((lstrcmp(lp1, L"x") == 0) || (lstrcmp(lp1, L"X") == 0))) goto mitex;
//	 parcount++;
  if (((*lp1 == L'/') || (*lp1 == L'-')))
	{
...
//			int     fixconsize = 0;
		   ...
		   if (ExtractFirstArgument(lp2, lp1, lp2)) {
				...
				WideCharToMultiByte(CP_ACP, 0, myMsg, -1, fixconfont, 32, NULL, NULL);  // statt 1250
			//	TrimString(fixconfont);
			};
        ...
        case L'U':
            ...
            if (lstrcmpi(lp1, L"UANSI") == 0) {
                fixmode = CPI_DEFAULT;
                flagSetEncoding = IDM_ENCODING_ANSI - IDM_ENCODING_ANSI + 1;
            };
            if (lstrcmpi(lp1, L"UOEM") == 0) {
                fixmode = CPI_OEM;
                flagSetEncoding = IDM_ENCODING_ANSI - IDM_ENCODING_ANSI + 1;
            };
					 ...
					 if (IsWindow(hparwindow)) {
						 ...
						 if (myconmod > 0) {
							 ...
							 if ((StrCmpN(myMsg, L"ConsoleW", 8)) == 0) {
								 ...
								 if (sogro > 0) {
									 fixconsize = sogro;
									 if (fixconsize < 200) fixconsize = fixconsize * myconmod ; // intern mal hundert
								 };
...
{
  WCHAR tch[MAX_PATH];
  if (GetPrivateProfileString(L"TU", L"N1.ini", L"", tch, COUNTOF(tch), lpszFile)) {
  ...
  if (*lpszIniFile) {
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
      if (GetFileSize(hFile,NULL) == 0) {
        DWORD dw;
        WriteFile(hFile,(LPCVOID)L"\xFEFF[TU]\r\n",26,&dw,NULL);
      }
      CloseHandle(hFile);
      return(1);
    }
