...
* TU - Text Editor
*
* Dialogs.c - Dialog boxes implementation
*
...
{
    // kommt in Filename . vor, wird 1 zu 1 genommen
    // sonst aus APPDATA nagy mit .rtf
    BOOL fSuccess = FALSE;

    int ilen = (int)wcslen(pszFile);
    for (; ilen > 1; ilen--) if (pszFile[ilen] == '.') break;
    WCHAR myFstri[256];
    if (ilen < 2) {
        SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, myFstri);
        wcscat(myFstri, L"\\nagy\\");
        wcscat(myFstri, pszFile);
        //  TrimString(myFstri);
        wcscat(myFstri, L".rtf");
    }
    else {
        lstrcpy(myFstri, pszFile);
    };


    HANDLE hFile = CreateFile(myFstri, GENERIC_READ,
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        WCHAR myMeld[256];
        wcscpy(myMeld, L"\rFile not found: ");
        wcscat(myMeld, pszFile);
        SendMessage(hwnd, WM_SETTEXT, 0, (LPARAM)myMeld);
    };
    ...
    HINSTANCE hinst)       // Application or DLL instance.
{
    ...
    HWND hwndRich = CreateWindowEx(0, MSFTEDIT_CLASS, L"",
        ES_AUTOVSCROLL | ES_MULTILINE | WS_VISIBLE | WS_CHILD | WS_TABSTOP | WS_HSCROLL | WS_VSCROLL ,
        x, y, width, height,
        hwndOwner, NULL, hinst, NULL);
    hwndRichedit = hwndRich;
    ...
    // ES_AUTOVSCROLL | ES_AUTOHSCROLL | ES_NUMBER | WS_BORDER | WS_VSCROLL |
    DWORD style = SES_HYPERLINKTOOLTIPS | SES_NOFOCUSLINKNOTIFY | SES_EXTENDBACKCOLOR;;
    SendMessage(hwndRich,EM_SETEDITSTYLE, style, style);


    //SendDlgItemMessage(hwnd, IDC_RICHEDITABOUT, EM_SETBKGNDCOLOR, 0, (LPARAM)GetBackgroundColor(hwnd));
    SendMessage(hwndEdit, EM_SETBKGNDCOLOR, 0, (LPARAM)GetSysColor(COLOR_INFOBK));
    SendMessage(hwndEdit, EM_SHOWSCROLLBAR, SB_VERT, TRUE);
    SendMessage(hwndEdit, EM_SETZOOM, 1, 1); //, 0, 0); // OFF

    FillRichEditFromFile(hwndEdit, L"lic.rtf");
    return hwndEdit;
}

int LoadRichEdit(int was) // Application or DLL instance.
{

    if (was == 2) {
        FillRichEditFromFile(hwndRichedit, L"cline.rtf");
        return 0;
    };
        FillRichEditFromFile(hwndRichedit, L"keys");
        return 0;
}



void n2e_ProcessAbout3rdPartyUrl( ENLINK* pENLink)
{
    if (pENLink->msg == WM_LBUTTONUP)
    {
        LPWSTR pUrl = (LPWSTR)malloc((pENLink->chrg.cpMax - pENLink->chrg.cpMin + 1) * sizeof(WCHAR));
    ...
    case WM_INITDIALOG:
      {
		...
		const wchar_t* pszTime = WIDEN(__TIME__);

        SetDlgItemText(hwnd, IDC_VERSION, VERSION_FILEVERSION_LONG );
		SetDlgItemText(hwnd, IDC_COPYRIGHT, VERSION_LEGALCOPYRIGHT_LONG);		
     //    SetDlgItemText(hwnd, IDC_AUTHORNAME, L"Florian Balmer");

  		lstrcpy(wch, L"mixed by f.nagy seit 2015, zuletzt am: ");
		lstrcat(wch, pszDate);
		lstrcat(wch, L" um: ");

        WCHAR twch[25];
        lstrcpy(twch, pszTime);

        twch[5] = L'\0';
        lstrcat(wch, twch);
        SetDlgItemText(hwnd, IDC_TIME, wch);

        HWND Redhwnd = GetDlgItem(hwnd, IDC_TIME);
        RECT rect, rectp;
        int height;
        height = 0;
        GetWindowRect(hwnd, &rectp);
        if (GetWindowRect(Redhwnd, &rect))
        {
            height = rect.bottom - rect.top;
        }

//        CreateRichEdit(hwnd, (rect.left - rectp.left), (rect.top - rectp.top + height), (rect.right - rect.left), (6 * height), g_hInstance);

        CreateRichEdit(hwnd, 8, (rect.top - rectp.top + 4), ((rectp.right - rectp.left) -  height), (9 * height), g_hInstance);
...
//	 	   SetDlgItemText(hwnd, IDC_WEBPAGE2, L"flo's freeware - http://www.flos-freeware.ch");
 //         ShowWindow(GetDlgItem(hwnd,IDC_WEBPAGE2),SW_SHOWNORMAL);
 //       }
 //       else {
//			wsprintf(wch, L"<A>%s</A>", L"flo's freeware - http://www.flos-freeware.ch");
			
  //      }

    
	/*	if (GetDlgItem(hwnd,IDC_EMAIL) == NULL) {
		  SetDlgItemText(hwnd, IDC_EMAIL2, L"www.frigyes.net");
          ShowWindow(GetDlgItem(hwnd,IDC_EMAIL2),SW_SHOWNORMAL);
        }
        else {
      //    wsprintf(wch,L"<A>%s</A>",L"Infos: www.nagy.in");
		  SetDlgItemText(hwnd, IDC_EMAIL, L"Infos: www.frigyes.net");
        }  */
    ...
    case WM_NOTIFY: {
        ...
        case EN_LINK:
         //   if (pnmhdr->idFrom == IDC_RICHEDIT) {
                n2e_ProcessAbout3rdPartyUrl( (ENLINK*)pnmhdr);
         //   }
         //   break;
        }
    }
...
INT_PTR CALLBACK RtfDlgProc(HWND hwnd, UINT umsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        HWND Redhwnd = GetDlgItem(hwnd, 107);
        RECT rect, rectp;
        int height;
        height = 0;
        GetWindowRect(hwnd, &rectp);
        if (GetWindowRect(Redhwnd, &rect))
       {
        ...
        int frame = GetSystemMetrics(SM_CYDLGFRAME);
        CreateRichEdit(hwnd, ( 2 * frame ) ,  ( 2 * frame ) , ((rectp.right - rectp.left) - 6 * frame ), rectp.bottom - rectp.top - capth - (2 * height) - ( 2 * frame) , g_hInstance);
        //        SendDlgItemMessage(hwnd, IDC_VERSION, WM_SETFONT, (WPARAM)hFontTitle, TRUE);
...
#ifdef oldoem
			  if (lstrcmpi(arg1, L"ty") == 0 ||
				  lstrcmpi(arg1, L"ty.exe") == 0) {
...
#endif
#ifdef neuutf
			  if (lstrcmpi(arg1, L"tu") == 0 ||
				  lstrcmpi(arg1, L"tu.exe") == 0) {
				  GetModuleFileName(NULL, arg1, COUNTOF(arg1));
				  bQuickExit = TRUE;
			  }
...
#ifdef winutf
              if (lstrcmpi(arg1, L"tw") == 0 ||
                  lstrcmpi(arg1, L"tw.exe") == 0) {
                  GetModuleFileName(NULL, arg1, COUNTOF(arg1));
                  bQuickExit = TRUE;
              }
#endif
...
void DisplayCmdLineHelp()
{
  ...
  //  GetString(IDS_CMDLINEHELP, szText, COUNTOF(szText));
    ...
    mbp.lpszText = L"start program,\n and show help (cmdline)";
