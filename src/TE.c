...
* TU - Text Editor
*
* TU.c - Main application window functionality
*
...
//#include "..\..\flexdDll\XConstDef.h"


    // 
 //   __declspec(dllimport)  int SetInt(int FirstValue, int SecondValue);
    //  __declspec(dllimport)  int SetStri (int welche, LPSTR a);
    //  __declspec(dllexport)  int SetProg (BYTE  procid, HWND mphwnd, LPSTR spname);
    //  // __declspec(dllimport)  int GetInt  (int FirstValue);
    //  // __declspec(dllimport)  int SetForg (HWND aTargetWindow) ;


/******************************************************************************
*
* Local and global Variables for TU.c
*
*/
HWND      hwndStatus;
// Toolbar removed
...
HWND      hwndMain;
#ifdef BUILD_TE
// Markdown Preview - defined in WebView2Helper.cpp
extern HWND hwndWebView;
extern BOOL bPreviewMode;
extern BOOL bStartInPreviewMode;
extern void WebView2Resize(int x, int y, int cx, int cy);
// RTF Preview - defined in Print.cpp
extern void RtfPreviewResize(int x, int y, int cx, int cy);
extern BOOL IsRtfPreviewMode(void);
extern BOOL IsHtmlPreviewMode(void);
#endif
...
boolean bnomenu     = FALSE;
boolean bToolw      = FALSE;
boolean bnowiparent = FALSE;
boolean bnowisend   = FALSE;
boolean letsaveini  = TRUE;
boolean mitf2com    = FALSE;
boolean bnoAsk       = FALSE;

boolean mittemp = FALSE;
boolean nochnicht = TRUE;
int tempcount = 0;

boolean flagOldOn   = FALSE;
boolean quickoff    = FALSE;
int fixenc = 0;

#ifdef winutf8
int fixmode = CPI_UTF8;
#endif
#ifdef winutf16
int fixmode = CPI_UNICODE;
#endif
...
// int     fixconsize = 1000;
// char    fixconfont[32] = "8514oem";
char   fixconfont[32] = "\0";
WCHAR myext[5];
WCHAR mymentxt[64] = L" ";
WCHAR mytempo[128] = L" ";

// Toolbar removed - tbbMainWnd array removed
...
// BOOL      bShowToolbar;  // Toolbar removed
BOOL      bShowStatusbar;
...
BOOL      flagbreadonly = FALSE;
int       iEncoding;
int       iOriginalEncoding;
int       iEOLMode;
BOOL      bSaveWithBOM = FALSE;
...
BOOL      bHighlightMatchingWords = TRUE;  // Enable word highlighting
#define INDIC_MATCHWORD 9  // Indicator number for matching words
void HighlightMatchingWords(HWND hwnd);  // Forward declaration
WCHAR     wchWndClass[16] = WC_TE;
...
// int flagToolbarLook        = 0;  // Toolbar removed
int flagSimpleIndentGuides = 0;
int fNoHTMLGuess           = 1;
int fNoCGIGuess            = 1;
int fNoFileVariables       = 1;
int flagPosParam           = 0;
...
int flagUseSystemMRU       = 1;
int flagRelaunchElevated   = 0;
int flagDisplayHelp        = 0;


void ErrorMeld(LPCTSTR lpszFunction, LPCTSTR lpszPar)
{
	WCHAR szBuf[380];
	WCHAR lpMsgBuf[480];
	...
	if (dw) wsprintf(szBuf, L"%s\n\n%s\nFehler Nummer: %d: %s", lpszFunction, lpszPar, dw, lpMsgBuf);
	 else wsprintf(szBuf, L"%s\n\n%s\n", lpszFunction, lpszPar);

	MessageBox(NULL, szBuf, NULL, MB_OK + MB_SYSTEMMODAL);
...
#pragma warning(suppress: 4996)  // GetVersion is deprecated but still useful here
 ...
 /* if (!Is2k()) {
    ...
    FormatMessage(
        ...
        0,
        NULL);
	MessageBox(NULL, (LPCWSTR)lpMsgBuf, WC_TE, MB_OK | MB_ICONEXCLAMATION);
    LocalFree(lpMsgBuf);
    return(0);
  } */

  // Check if running with elevated privileges
  //  fIsElevated = IsElevated(); not elevated
  ...
  i = (int)(wcslen(lpFileArg));
  lstrcpy(myext, L"");

  // die rechte 3 Zeichen als myext
  if (i > 4) {
	  lstrcpy(myext, lpFileArg + i - 3);
	  if (myext[0] == '.') {
		  myext[0] = myext[1];
		  myext[1] = myext[2];
		  myext[2] = L'\0';
	  ...
	  // ini setzen mit one char => also nach erweiterung



  if ((wcslen(szIniFile)) == 1) {  // ini setzen mit one char => also nach erweiterung
	                               // h(ome, f(2p, d(ocument, n(agy in app-local


	  if (!(_wcsicmp(szIniFile, L"h"))) {  // home
		  dwRet = GetEnvironmentVariable(L"home", envva, 250);
		  if (dwRet != 0) {
			  lstrcpy(tchModule, envva);
			  wcscat(tchModule, L"\\");
			  wcscat(tchModule, myext);
			  wcscat(tchModule, L".ini");
	...
	// 0x2E414759
	  ...
	  if (!(_wcsicmp(szIniFile, L"d"))) {  // wo doc liegt ext.ini
		  lstrcpy(tchModule, lpFileArg);
	  ...
	  if (!(_wcsicmp(szIniFile, L"n"))) {  // aus win appdata local
		  SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, szIniFile);
		  wcscat(szIniFile, L"\\nagy\\");
		  wcscat(szIniFile, myext);
	  ...
	  if (!(_wcsicmp(szIniFile, L"f")) ) lstrcpy(szIniFile, myext);

  };   //  end one char ini


   if  (wcslen(szIniFile))  {  // pr�fen ob ini ohne pfad (bzw. .ini )
                               // zB. f2t

          lp1 = (StrChr(szIniFile, L'\\'));
		  if ( lp1 == NULL ) {  // ini ohne pfad


		  lp1 = (StrChr(szIniFile, L'.'));
		  if (lp1 == NULL) {  // ini ohne pfad

		  lstrcpy(tchModule, wchWorkingDirectory);
		  wcscat(tchModule, L"\\");
		  wcscat(tchModule, szIniFile);
		  wcscat(tchModule, L".ini");
		  lstrcpy(szIniFile, tchModule);
		  letsaveini = FALSE;
	                       	  }
 ...
 // als allerletzte ---
  ...
  if ( (wcslen(szIniFile)) < 4 ) {  // standard = Filename.ini in exe tinis ordner
                                    // also  bei \flex\ty.exe \flex\tinis\ty.ini


	  GetModuleFileName(NULL, tchModule, COUNTOF(tchModule));  // exe Path
	  lstrcpy(tchModule, PathFindFileName(tchModule));         // davon nur Fileanme

	  PathRenameExtension(tchModule, L".ini");                // 

	  SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, szIniFile);
	  wcscat(szIniFile, L"\\nagy\\");
	  wcscat(szIniFile, tchModule);
	  TrimString(szIniFile);
  }
  else {

	  lp1 = (StrChr(szIniFile, '\\'));
	  if (lp1 == NULL) {  // ini ohne pfad
		  lstrcpy(tchModule, szIniFile);
		  SHGetFolderPath(NULL, CSIDL_LOCAL_APPDATA, NULL, SHGFP_TYPE_CURRENT, szIniFile);
		  wcscat(szIniFile, L"\\nagy\\");
		  wcscat(szIniFile, tchModule);
		  TrimString(szIniFile);
...
*/
 ...
 //   return(0);
  ...
  hModUxTheme = LoadLibrary(L"uxtheme.dll");  // Für schöne Controls
  ...
  bReadOnly = flagbreadonly;
  hAccMain = LoadAccelerators(hInstance,MAKEINTRESOURCE(IDR_MAINWND));
  hAccFindReplace = LoadAccelerators(hInstance,MAKEINTRESOURCE(IDR_ACCFINDREPLACE));

//  Margin setting 
  SendMessage(hwndEdit, SCI_SETMARGINTYPEN, 1, SC_MARGIN_SYMBOL);
  SendMessage(hwndEdit, SCI_SETMARGINWIDTHN, 1, 20);
/*

  for (int i = 1; i < 25; i++)   SendMessage(hwndEdit, SCI_MARKERDEFINE, i, i);

  SendMessage(hwndEdit, SCI_MARKERSETFORE, 3, RGB(0x0A, 0x24, 0x6A));
  SendMessage(hwndEdit, SCI_MARKERSETBACK, 3, RGB(0xFF, 0xFF, 0x00));
  SendMessage(hwndEdit, SCI_MARKERSETFORE, 4, RGB(0xFF, 0x00, 0x00));
  SendMessage(hwndEdit, SCI_MARKERSETBACK, 4, RGB(0xFF, 0x00, 0x00));

  for (int i = 1; i < 25; i++)    SendMessage(hwndEdit, SCI_MARKERADD, i, i);
  


 // MsgBox(MBWARN, IDT_EDIT_FIND, L"ich starte jetzt...\n\dasdsd");

  /************************* Anzeigen mit Turkline 
  SendMessage(hwndEdit, SCI_SETMARGINTYPEN, 1, SC_MARGIN_BACK);
//  SendMessage(hwndEdit, SCI_SETMARGINTYPEN, 1, SC_MARGIN_SYMBOL);
//  SendMessage(hwndEdit, SCI_SETMARGINWIDTHN, 1, 20);
//  SendMessage(hwndEdit, SCI_MARKERSETFORE, 3, RGB(0xFF, 0xFF, 0x0));
  SendMessage(hwndEdit, SCI_MARKERSETBACK, 3, RGB(0xF0, 0x0, 0x80));
  SendMessage(hwndEdit, SCI_MARKERADD, 2, 3);
  SendMessage(hwndEdit, SCI_MARKERADD, 5, 3);

 */

  /* Style setting
  SendMessage(hwndEdit, SCI_INDICSETSTYLE, 1, 4);
  SendMessage(hwndEdit, SCI_SETINDICATORCURRENT, 1, 0);
  SendMessage(hwndEdit, SCI_INDICSETFORE, 1, RGB(0xFF, 0x00, 0x00));
 // SendMessage(hwndEdit, SCI_INDICSETALPHA, 1, 200);
  SendMessage(hwndEdit, SCI_INDICATORFILLRANGE, 11, 30);
  SendMessage(hwndEdit, SCI_INDICATORFILLRANGE, 60, 80);
  */
...
BOOL InitApplication(HINSTANCE hInstance)
{
  ...
  else  wc.lpszMenuName  = MAKEINTRESOURCE(IDR_MAINWND);
...
HWND InitInstance(HINSTANCE hInstance,LPSTR pszCmdLine,int nCmdShow)
{
  ...
  MONITORINFO mi;
  DWORD myStyle = WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN;
  HWND mywin = NULL;
...
//  if (bnomenu) myStyle = WS_EX_CONTEXTHELP| WS_EX_CLIENTEDGE | WS_EX_DLGMODALFRAME | WS_SIZEBOX | WS_MAXIMIZEBOX | WS_SYSMENU;
  if (!bnowiparent) mywin = hparwindow;   

  if (bToolw) myStyle = WS_EX_TOOLWINDOW | WS_SYSMENU; // | WS_SIZEBOX | WS_MAXIMIZEBOX ;

  hwndMain = CreateWindowEx(
	 // WS_EX_TOOLWINDOW,
	// WS_EX_TOPMOST | WS_EX_DLGMODALFRAME | WS_EX_TOOLWINDOW | WS_EX_WINDOWEDGE,
	  0,
	  wchWndClass,
	  WC_TE,
	  myStyle,
	  //  WS_EX_TOOLWINDOW | WS_EX_DLGMODALFRAME | WS_SIZEBOX | WS_SYSMENU,
	  //		   WS_EX_PALETTEWINDOW,
  ...
  // Force foreground - Windows blocks SetForegroundWindow from background processes
  DWORD dwForeThread = GetWindowThreadProcessId(GetForegroundWindow(), NULL);
  DWORD dwThisThread = GetCurrentThreadId();
  if (dwForeThread != dwThisThread) {
    AttachThreadInput(dwForeThread, dwThisThread, TRUE);
    SetForegroundWindow(hwndMain);
    BringWindowToTop(hwndMain);
    AttachThreadInput(dwForeThread, dwThisThread, FALSE);
  } else {
    SetForegroundWindow(hwndMain);
  }
  ...
  if (lpFileArg /*&& !flagNewFromClipboard*/)
  {
    ...
    // Wildcard pattern (e.g. "test*.log" or "C:\temp\*.txt")
    else if (wcspbrk(lpFileArg, L"*?")) {
      WCHAR tchFile[MAX_PATH];
      WCHAR tchDir[MAX_PATH];
      WCHAR tchPattern[MAX_PATH];
      lstrcpyn(tchDir, lpFileArg, COUNTOF(tchDir));
      lstrcpyn(tchPattern, PathFindFileName(lpFileArg), COUNTOF(tchPattern));
      PathRemoveFileSpec(tchDir);
      if (!lstrlen(tchDir))
        lstrcpy(tchDir, g_wchWorkingDirectory);
      if (OpenFileDlgWithPattern(hwndMain,tchFile,COUNTOF(tchFile),tchDir,tchPattern))
        bOpened = FileLoad(FALSE,FALSE,FALSE,FALSE,tchFile);
    }
    else {
...
#ifdef BUILD_TE
  // Activate preview mode if /V was specified
  if (bStartInPreviewMode) {
    ToggleMarkdownPreview(hwndMain);
  }
#endif
...
// lagSetEncoding = IDM_ENCODING_ANSI + 1;
  // 
  ...
  if (flagMatchText && lpMatchArg) {
    ...
    if (lstrlen(lpMatchArg) && SendMessage(hwndEdit,SCI_GETLENGTH,0,0)) {
 	...
 	UINT cp = (UINT)SendMessage(hwndEdit, SCI_GETCODEPAGE, 0, 0);
	  ...
	  if (flagMatchText == 16) {
		  // suchen nach ###>> in Dataflex.PRN
          
		  int iMaxLine = 0;
		  char hello[25];

		  if (lpMatchArg[0] != '>') iMaxLine = (_wtoi(lpMatchArg));
		  bHiliteCurrentLine = TRUE;
		  Style_SetCurrentLineBackground(hwndEdit);


		  WideCharToMultiByte(cp, 0, lpMatchArg, -1, hello, 21, NULL, NULL);

		  if (!iMaxLine) {
			  bMarkLongLines = 1;
			  SendMessage(hwndEdit, SCI_SETEDGEMODE, 1, 0);
			  Style_SetLongLineColors(hwndEdit);
			  SendMessage(hwndEdit, SCI_SETEDGECOLUMN, (strlen(hello)) - 1, 0);


		  };
		  SendMessage(hwndEdit, SCI_SETMARGINTYPEN, 1, SC_MARGIN_BACK);
		  SendMessage(hwndEdit, SCI_MARKERSETBACK, 3, RGB(0x20, 0xD0, 0x20));

		  // strcpy(hello, lpMatchArg);

		  // SCI_GETFLEXL was a custom extension, not available in Scintilla 5.x
		  // iMaxLine = (int)SendMessage(hwndEdit, SCI_GETFLEXL, iMaxLine, (LPARAM)hello);
		  iMaxLine = 0; // TODO: implement alternative search
		  
		  if (iMaxLine )  EditJumpTo(hwndEdit, (abs( iMaxLine)), 1);
...
LRESULT CALLBACK MainWndProc(HWND hwnd,UINT umsg,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(umsg)
  {
    ...
    case WM_CLOSE:
	    ...
	    if (bnowisend) break;
	    if (hparwindow != NULL) {
		   if (IsWindow(hparwindow)) {
		  	    if (SetForegroundWindow(hparwindow)) {
				  SetActiveWindow(hparwindow);
				  SetFocus(hparwindow);
			  };
    ...
    case WM_CONTEXTMENU:
    {
      ...
      if ((nID != IDC_EDIT) && (nID != IDC_STATUSBAR))  // Toolbar removed
        return DefWindowProc(hwnd,umsg,wParam,lParam);

	  imenu = IDR_POPUPMENU;
	  if (quickoff)   imenu = IDR_POPUPMENU1;
	  hmenu = LoadMenu(g_hInstance, MAKEINTRESOURCE(imenu));
  /*    SetMenuDefaultItem(GetSubMenu(hmenu,9),0,FALSE);
	  ...
	  DeleteMenu(hmenu, 0, MF_BYPOSITION);
	  if (quickoff)  {
		  //  imenu = IDR_POPUPMENU1;
/*		  DeleteMenu(hmenu, 4, 0);
		  DeleteMenu(hmenu, 4, 0);
		  DeleteMenu(hmenu, 4, 0);
		  DeleteMenu(hmenu, 4, 0);

		  ModifyMenu(hmenu, IDM_FILE_PRINTF12, MF_STRING, IDM_FILE_PRINTF12, L"Zeile �bernehmen \tF12");
		  ModifyMenu(hmenu, IDM_FILE_PRINTF11, MF_STRING, IDM_FILE_PRINTF11, L"Zeile �bernehmen \tF11");
		  ModifyMenu(hmenu, IDM_FILE_PRINT, MF_STRING, IDM_FILE_PRINT, L"Restart last Prog\tF2");
		  ModifyMenu(hmenu, IDM_FILE_PRINTF2, MF_STRING, ID_FILE_RF2, L"Zeile �bernehmen \tF2");
	  };
		*/
      ...
      switch(nID)
      {
        ...
        // Toolbar removed - IDC_TOOLBAR and IDC_REBAR cases removed
        case IDC_STATUSBAR:
          if (pt.x == -1 && pt.y == -1)
            GetCursorPos(&pt);
          imenu = 1;
    ...
    case WM_SYSCOMMAND:
     ...
     //     if (!bRunningWatch)      InstallFileWatching(szCurFile);
        break;


    //// This message is posted before TU reactivates itself
...
//  Call(SCI_SETMARGINTYPEN, 1, SC_MARGIN_SYMBOL);
 // Call(SCI_SETMARGINWIDTHN, 1, 20);
  ...
  // Create Statusbar (Toolbar removed)
  ...
  if (hwndEdit == NULL || hwndEditFrame == NULL ||
      hwndStatus == NULL)  // Toolbar removed
    return(-1);

// WebView2 is now initialized on-demand when first needed (lazy loading)
...
//=============================================================================
//
//  CreateBars() - Create Statusbar (Toolbar removed)
//
//
void CreateBars(HWND hwnd,HINSTANCE hInstance)
{
  ...
  // No ReBar needed without Toolbar
  cyReBar = 0;
  cyReBarFrame = 0;
}
...
void MsgThemeChanged(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  // recreate statusbar (Toolbar removed)
...
void MsgSize(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  // Toolbar removed - cyReBar is always 0 now
...
#ifdef BUILD_TE
  // Resize WebView2 for Markdown Preview
  if (hwndWebView) {
    WebView2Resize(x, y, cx, cy);
  }
  // Resize RichEdit for RTF Preview
  RtfPreviewResize(x, y, cx, cy);
#endif
...
void MsgInitMenu(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  // Enable "Run CPS Script" only for .cps/.cpu/.cpy/.cpx files
  {
    LPCWSTR ext = PathFindExtension(szCurFile);
    EnableCmd(hmenu, IDM_FILE_RUNCPS,
      i && ext && (_wcsicmp(ext, L".cps") == 0 ||
  ...
  if (flagbreadonly) {
      EnableCmd(hmenu, IDM_FILE_READONLY, FALSE);
      CheckCmd(hmenu,  IDM_FILE_READONLY, TRUE );

  }
  else {
      EnableCmd(hmenu, IDM_FILE_READONLY, i);
      CheckCmd(hmenu, IDM_FILE_READONLY, flagbreadonly);
  };
...
/*  EnableCmd(hmenu,IDM_ENCODING_RECODE,i);
  ...
  EnableCmd(hmenu,IDM_EDIT_CUT,1 /*&& !bReadOnly*/);  // Always enabled - cuts line if no selection
  EnableCmd(hmenu,IDM_EDIT_COPY,1 /*&& !bReadOnly*/);
 ...
 // SendMessage(hwndEdit,SCI_LINEFROMPOSITION,(WPARAM)SendMessage(hwndEdit,SCI_GETSELECTIONSTART,0,0),0) >= 1);
  ...
  EnableCmd(hmenu,IDM_EDIT_STREAMCOMMENT,
    !(i == SCLEX_NULL || i == SCLEX_MAKEFILE || i == SCLEX_ASM ||
 ...
 // EnableCmd(hmenu,IDM_VIEW_CUSTOMIZETB,bShowToolbar);
  ...
  i = 1;
  dwRet = GetEnvironmentVariable(L"dfroot", envva, 5);
  if (0 == dwRet) i = 1;
  
  EnableCmd(hmenu, ID_FILE_RF2, mitf2com);

  if (mitf2com) {
      if ((CSTRLEN(mymentxt)) > 3) {
          ModifyMenu(hmenu, ID_FILE_RF2, MF_STRING, ID_FILE_RF2, mymentxt);
      }
      else {
          if (flagOldOn) ModifyMenu(hmenu, ID_FILE_RF2, MF_STRING, ID_FILE_RF2, L"Restart last Prog\tF2");
          if (quickoff)  ModifyMenu(hmenu, ID_FILE_RF2, MF_STRING, ID_FILE_RF2, L"Zeile �bernehmen und beenden\tF2");
      };
  };
...
//  i = (lstrlen(szIniFile) > 0 || lstrlen(2) > 0); ???
  i = (lstrlen(szIniFile) > 0 );
  EnableCmd(hmenu, IDM_VIEW_SAVESETTINGSNOW, i);

}

WCHAR *wsrepl(WCHAR *string, WCHAR *oldpiece, WCHAR *newpiece) {

	int str_index, newstr_index, oldpiece_index = 0;
	int new_len, old_len, cpy_len, end;
	WCHAR *c;
	static WCHAR newstring[250];

	if ((c = (WCHAR *)wcsstr(string, oldpiece)) == NULL)  {
		WCHAR *news;
		WCHAR *newpiece_lower;
		news = _wcslwr(_wcsdup(string));
		newpiece_lower = _wcslwr(_wcsdup(oldpiece));
		c = (WCHAR *)wcsstr(news, newpiece_lower);
		if (c != NULL)  oldpiece_index = (int)(c - news);
		free(news);
		free(newpiece_lower);
		if (c == NULL)   return string;

	}
	else oldpiece_index = (int)(c - string);

	new_len = (int)wcslen(newpiece);
	old_len = (int)wcslen(oldpiece);
	end = (int)wcslen(string) - old_len;
	...
	{
		// f�r replace all
		/* Copy WCHARacters from the left of matched pattern occurence */
		cpy_len = oldpiece_index - str_index;
		wcsncpy(newstring + newstr_index, string + str_index, cpy_len);
		newstr_index += cpy_len;
		str_index += cpy_len;

		/* Copy replacement WCHARacters instead of matched pattern */
		lstrcpy(newstring + newstr_index, newpiece);
		newstr_index += new_len;
		str_index += old_len;

		/* Check for another pattern match */
		if ((c = (WCHAR *)wcsstr(string + str_index, oldpiece)) != NULL)
			oldpiece_index = (int)(c - string);

	}

	/* Copy remaining WCHARacters from the right of last matched pattern */
	lstrcpy(newstring + newstr_index, string + str_index);
	return newstring;
};


// Erstellt Versionskopie: test.cps -> test_1.cps, test_2.cps, etc.
// Gibt den Backup-Pfad in backupPath zurueck
BOOL CreateVersionBackup(LPCWSTR pszFile, LPWSTR backupPath, int cchMax)
{
	...
	// Zerteile in Verzeichnis, Name, Extension
	lstrcpy(szDir, pszFile);
	PathRemoveFileSpec(szDir);

	lstrcpy(szName, PathFindFileName(pszFile));
	lstrcpy(szExt, PathFindExtension(szName));
	PathRemoveExtension(szName);

	// Prüfe ob Name auf _N endet (z.B. test_3) -> Basisname extrahieren
	// Aber test_ (ohne Zahl) ist Testmodus-Flag, nicht anfassen
	lstrcpy(szBase, szName);
	{
		int len = lstrlen(szBase);
		if (len > 1 && szBase[len - 1] != L'_') {
			// Suche rueckwaerts nach _Zahl
			int i = len - 1;
			while (i > 0 && szBase[i] >= L'0' && szBase[i] <= L'9')
				i--;
			if (i > 0 && i < len - 1 && szBase[i] == L'_') {
				// Endet auf _N -> abschneiden
				szBase[i] = L'\0';
			}
		}
	}

	// Finde naechste freie Nummer
	for (n = 1; n <= 999; n++) {
		wsprintf(backupPath, L"%s\\%s_%i%s", szDir, szBase, n, szExt);
		if (!PathFileExists(backupPath))
			break;
	}
	if (n > 999) return FALSE;

	return CopyFile(pszFile, backupPath, FALSE);
}


int xfilesave(int mypar1)
{
	FILE *fh;
	WCHAR szF2cPath[MAX_PATH + 4];
	WCHAR backupPath[MAX_PATH + 4];
	int iPos, iLn, iCol;

	if (!lstrlen(szCurFile))
		return 0;

	// 1. Datei speichern (erzwungen, ohne Fragen)
	FileSave(TRUE, FALSE, FALSE, FALSE);

	// 2. Versionskopie erstellen
	backupPath[0] = L'\0';
	CreateVersionBackup(szCurFile, backupPath, MAX_PATH);

	// 3. Cursorposition ermitteln
	...
	// 4. Comm-Datei (.f2c) neben der Skript-Datei schreiben
	lstrcpy(szF2cPath, szCurFile);
	PathRemoveExtension(szF2cPath);
	lstrcat(szF2cPath, L".f2c");

	if ((fh = _wfopen(szF2cPath, L"wt")) != NULL)
	{
		fputws(L"[f2c]\n", fh);
		fputws(L"action=restart\n", fh);

		fwprintf(fh, L"file=%s\n", szCurFile);

		if (backupPath[0])
			fwprintf(fh, L"backup=%s\n", backupPath);

		fwprintf(fh, L"line=%i\n", iLn);
...
/* startmyed removed - f2p no longer used */
...
int myRunp(LPCWSTR myCline, WCHAR par2)
{
	STARTUPINFO si;
    ...
    if (par2 > 'a') {
        ...
        WCHAR wchWorkingDirectory[MAX_PATH];
        WCHAR wchExePath[MAX_PATH];
        GetModuleFileName(NULL, wchExePath, COUNTOF(wchExePath));
        wcscpy(wchWorkingDirectory, wchExePath);
        PathRemoveFileSpec(wchWorkingDirectory);

        wcscpy(envva, wchWorkingDirectory);
        // Always use TE.exe to edit INI (UTF-16 LE editor)
        wcscpy(lstring, L"\"");
        wcscat(lstring, wchWorkingDirectory);
        wcscat(lstring, L"\\TE.exe\" /e unicode \"");
        wcscat(lstring, szIniFile);
        wcscat(lstring, L"\"");
        // Don't keep working directory locked
     //   GetCurrentDirectory(COUNTOF(g_wchWorkingDirectory), g_wchWorkingDirectory);
     //   GetModuleFileName(NULL, wchWorkingDirectory, COUNTOF(wchWorkingDirectory));
...
//	_wsplitpath(curexedir, drive, mdir, fname, ext);
//	_wmakepath(lstring, drive, mdir, L"f2p", L"exe");
	...
	if (CreateProcess(NULL, // No module name (use command line). 
		...
		envva,             // Use parent's starting directory. 
		...
		)
	{
		...
		if (flagReuseWindow) {
			WaitForInputIdle(pi.hProcess, waitt);
			return(0);
		};

		WaitForSingleObject(pi.hProcess, waitt);
		return(0);

	}

	MsgBox(MBWARN, IDS_ERR_LOADFILE, lstring);
	return(1);
...
LRESULT MsgCommand(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(LOWORD(wParam))
  {
    ...
    case IDM_FILE_REVERT:
      {
        if (lstrlen(szCurFile)) {
          ...
          if ((bModified ) && MsgBox(MBOKCANCEL,IDS_ASK_REVERT) != IDOK)
            return(0);
    ...
    //      iWeakSrcEncoding = iEncoding;
		//  if (FileLoad(TRUE, FALSE, TRUE, TRUE, tchCurFile2))
    ...
    case IDM_FILE_READONLY:
      ...
      if (lstrlen(szCurFile))
      {
        ...
        if (flagbreadonly) bReadOnly = TRUE;
    ...
    case IDM_FILE_BROWSE:  // als edit Prop 
      {
        ...
        WCHAR tchExeFile[MAX_PATH+4];
			...
			dwRet = GetEnvironmentVariable(L"dfroot", envva, 250);
			if (0 == dwRet) {
		//		MsgBox(MBWARN, IDS_ERR_LOADFILE, L"DFROOT ist nicht gesetzt!\nso kann ich mx.exe nicht starten");
		//		break;
                wcscpy(envva,L"C:\\dev");

			};



			wcscpy(tchExeFile, envva);
            wcscat(tchExeFile, L"\\tw.exe");
            wcscpy(tchParam,L" -Yl \"");
            wcscat(tchParam, szIniFile);
            wcscat(tchParam,L"\"");
    ...
    case IDM_FILE_RUNCPS:
      {
        WCHAR tchCpsExe[MAX_PATH];
        WCHAR tchDirectory[MAX_PATH] = L"";
        WCHAR tchParams[MAX_PATH+4];
        LPCWSTR ext;

        if (!lstrlen(szCurFile))
          break;

        // Only for .cps/.cpu/.cpy/.cpx files
        ext = PathFindExtension(szCurFile);
...
/*
    ...
    case IDM_ENCODING_SELECT:
      {
        ...
        if (LOWORD(wParam) == IDM_ENCODING_SELECT && !SelectEncodingDlg(hwnd,&iNewEncoding))
          ...
          switch (LOWORD(wParam)) {
            ...
            case IDM_ENCODING_UNICODEREV: iNewEncoding = CPI_OEM; break; // CPI_UNICODEBEBOM; break;
    ...
    case IDM_EDIT_CUT:
      if (flagPasteBoard)
        bLastCopyFromMe = TRUE;
      // If nothing selected, cut the entire line
...
//          EndWaitCursor();
          break;
        case SCLEX_MAKEFILE:  // es ist f2t
                        break;
...
//          break;
    ...
    case IDM_EDIT_FIND:
      {
        // Copy selected text to find field if there's a selection
        int iSelStart = (int)SendMessage(hwndEdit,SCI_GETSELECTIONSTART,0,0);
        int iSelEnd = (int)SendMessage(hwndEdit,SCI_GETSELECTIONEND,0,0);
        if (iSelEnd > iSelStart && (iSelEnd - iSelStart) < 512) {
          char szSel[512];
          SendMessage(hwndEdit,SCI_GETSELTEXT,0,(LPARAM)szSel);
          // Only use if no newlines in selection
          if (!StrChrA(szSel,'\r') && !StrChrA(szSel,'\n')) {
            lstrcpynA(efrData.szFind,szSel,COUNTOF(efrData.szFind));
            // Also update UTF8 version
            UINT cp = (UINT)SendMessage(hwndEdit,SCI_GETCODEPAGE,0,0);
            if (cp == SC_CP_UTF8) {
              lstrcpynA(efrData.szFindUTF8,szSel,COUNTOF(efrData.szFindUTF8));
            } else {
              WCHAR wch[512];
              MultiByteToWideChar(cp,0,szSel,-1,wch,COUNTOF(wch));
              WideCharToMultiByte(CP_UTF8,0,wch,-1,efrData.szFindUTF8,COUNTOF(efrData.szFindUTF8),NULL,NULL);
            }
          }
        }
    ...
    // IDM_VIEW_SCHEMECONFIG removed - Style editor not needed
    ...
    // Toolbar removed - IDM_VIEW_TOOLBAR and IDM_VIEW_CUSTOMIZETB handlers removed
  ...
  //    if (ChangeNotifyDlg(hwnd))        InstallFileWatching(szCurFile);
  //    break;
	...
	case IDM_VIEW_SAVESETTINGSNOW:
	{
									 ...
									 if (!bCreateFailure) {
										 if (!letsaveini) {
											 MsgBox(MBINFO, IDS_SETTINGSNOTSAVED, szIniFile);
											 break;
										 };

										 if (MsgBox(MBYESNO, IDS_ASK_SAVE, szIniFile) == IDYES) {
    ...
    case IDM_VIEW_RESETSTYLES:
      if (lstrlen(szIniFile) > 0) {
        if (MessageBox(hwnd, L"Reset all styles to defaults?\n\nTE will restart.", L"TE", MB_YESNO | MB_ICONQUESTION) == IDYES) {
          WCHAR szExe[MAX_PATH];
          // Delete style sections from INI
          WritePrivateProfileSection(L"Default Text", NULL, szIniFile);
          WritePrivateProfileSection(L"XML Document", NULL, szIniFile);
          WritePrivateProfileSection(L"JSON Document", NULL, szIniFile);
          WritePrivateProfileSection(L"Flex2Text", NULL, szIniFile);
          WritePrivateProfileSection(L"SQL Query", NULL, szIniFile);
          WritePrivateProfileSection(L"Curl Program Script", NULL, szIniFile);
          WritePrivateProfileSection(L"Batch Files", NULL, szIniFile);
          WritePrivateProfileSection(L"Markdown", NULL, szIniFile);
          WritePrivateProfileSection(L"Styles", NULL, szIniFile);
          WritePrivateProfileSection(L"Custom Colors", NULL, szIniFile);
          // Restart TE
        ...
        MessageBox(hwnd, L"No INI file configured.\n\nStyles are already at defaults.", L"TE", MB_OK | MB_ICONINFORMATION);
      }
      break;

    case IDM_VIEW_RESETSETTINGS:
      if (lstrlen(szIniFile) > 0) {
        if (MessageBox(hwnd, L"Reset ALL settings to defaults?\n\nINI file will be deleted and TE will restart.", L"TE", MB_YESNO | MB_ICONQUESTION) == IDYES) {
          WCHAR szExe[MAX_PATH];
          // Delete entire INI file
        ...
        MessageBox(hwnd, L"No INI file configured.\n\nSettings are already at defaults.", L"TE", MB_OK | MB_ICONINFORMATION);
      }
      break;

    case CMD_RELOADANSI:
        ThemedDialogBox(g_hInstance, MAKEINTRESOURCE(IDD_STYLECONFIG),
            hwnd, RtfDlgProc);
        break;
...
/*
    ...
    case CMD_RELOADASCIIASUTF8:
      {
		...
		BOOL bLoadASCIIasUTF8 = _bLoadASCIIasUTF8;
		fixencode = 0;
	}
      break;
    ...
    case CMD_RELOADNOFILEVARS:
      {
        ...
        if (lstrlen(szCurFile)) {
			...
			if (lstrlen(szCurFile)) {
				iSrcEncoding = CPI_UNICODE;  // da setzti
				lstrcpy(tchCurFile2, szCurFile);
				FileLoad(FALSE, FALSE, TRUE, FALSE, tchCurFile2);
				UpdateStatusbar();
			}
    ...
    // IDT_VIEW_SCHEMECONFIG removed - Style editor not needed
    ...
    case IDT_EDIT_CLEAR:
   //   if (IsCmdEnabled(hwnd,IDM_EDIT_CLEAR))  //my refresh
		  SendMessage(hwnd, WM_COMMAND, MAKELONG(IDM_FILE_REVERT, 1), 0);
   //   else
	//	  SendMessage(hwndEdit, IDM_FILE_REVERT, 0, 0);
      break;
...
LRESULT MsgNotify(HWND hwnd,WPARAM wParam,LPARAM lParam)
{
  ...
  switch(pnmh->idFrom)
  {
        ...
        case SCN_UPDATEUI:
          ...
          if (scn->updated & ~(SC_UPDATE_V_SCROLL | SC_UPDATE_H_SCROLL)) {
            ...
            if (scn->updated & SC_UPDATE_SELECTION) {
              HighlightMatchingWords(hwndEdit);
            }
            ...
            if (bMatchBraces)
            {
              ...
              if (iEndStyled < (int)SendMessage(hwndEdit,SCI_GETLENGTH,0,0)) {
                ...
                int iStartStyled = (int)SendMessage(hwndEdit,SCI_POSITIONFROMLINE,iLine,0);
                SendMessage(hwndEdit,SCI_COLOURISE,iStartStyled,-1);
              }
    ...
    // Toolbar removed - IDC_TOOLBAR notification handler removed
    ...
    default:
      ...
      switch(pnmh->code)
      {
        ...
        case EN_LINK:
          {
            // Handle link clicks in RTF preview
            ...
            if (pEnLink->msg == WM_LBUTTONUP) {
              ...
              tr.lpstrText = szUrl;
              if (tr.chrg.cpMax - tr.chrg.cpMin < 500) {
                if (SendMessage(pnmh->hwndFrom, EM_GETTEXTRANGE, 0, (LPARAM)&tr) > 0) {
...
void LoadSettings()
{
  ...
  bSaveRecentFiles =
    IniSectionGetInt(pIniSection,L"SaveRecentFiles",1);
  ...
  bMatchBraces = IniSectionGetInt(pIniSection,L"MatchBraces",0);
  ...
  bAutoIndent = IniSectionGetInt(pIniSection,L"AutoIndent",0);
  ...
  bTabsAsSpaces = IniSectionGetInt(pIniSection,L"TabsAsSpaces",0);
  ...
  iTabWidth = IniSectionGetInt(pIniSection,L"TabWidth",1);
  iTabWidth = max(min(iTabWidth,256),1);
  iTabWidthG = iTabWidth;
  ...
  bShowLineNumbers = IniSectionGetInt(pIniSection,L"ShowLineNumbers",0);
  ...
  // bisschen cheese, warum w�chselt Encoding von 5 auf 1 ???
#ifdef winutf8
  ...
  // Print functionality removed - using external print program
  /*
...
//  if (bResetFileWatching) bResetFileWatching = 1;
  ...
  bTransparentModeAvailable = 0;
  //  (GetProcAddress(GetModuleHandle(L"User32"),"SetLayeredWindowAttributes") != NULL);

  // Toolbar removed - ToolbarButtons and ShowToolbar settings removed
...
*/
  ...
  {
  //  int acp = GetACP();  brauchen wir in Fernosten???
...
void SaveSettings(BOOL bSaveSettingsNow)
{
  ...
  if (!bSaveSettings && !bSaveSettingsNow) {
    ...
    IniSetInt(L"Settings",L"SaveRecentFiles",bSaveRecentFiles);
    IniSetInt(L"Settings",L"SaveFindReplace",bSaveFindReplace);
    return;
  }
...
//			prc->top = 0;
	...
	if ((tolower(meinch)) == L'r') {
		if (meinch == L'r') xbreit = (int)(xbreit / 1.8);
	...
	if ((tolower(meinch)) == L'l') {
		if (meinch == L'l') xbreit = (int)(xbreit / 1.8);
		xoben = prc->left;
		return;
...
//  ParseCommandLine()
//
// zB: /yl  /f nagy\f2tdf.ini /J Consolas /J10 c:\dev\erg.F2T
...
#ifdef oldoem
                            // ???? OEM
 // flagSetEncoding = IDM_ENCODING_OEM - IDM_ENCODING_ANSI + 1;
  ...
  if (((*lp1 == L'/') || (*lp1 == L'-')))
	{
		...
		case L'I':
            ...
            for (int ili = 1; lp1[ili] != L'\0'; ili++) {
                par2 = (WCHAR)(ULONG_PTR)CharUpper((LPWSTR)(ULONG_PTR)lp1[ili]);
                if (par2 == L'0') bToolw = TRUE;
                if (par2 == L'-') bnomenu = TRUE;
                if (par2 == L'^') bnowiparent = TRUE;
                if (par2 == L'~') bnowisend = TRUE;
                if (par2 == L'N') bnoAsk = TRUE;
                if (par2 == L'R') flagbreadonly = TRUE;;
            };
			break;

		case L'J':
			if (*(lp1 + 1) >= L'0' && *(lp1 + 1) <= L'9') {
				fixconsize = _wtoi(lp1 + 1);
				if (fixconsize > 0) {
					if (fixconsize < 200) fixconsize = fixconsize * 100; // intern mal hundert
				};
				break;
    			}

		   if (ExtractFirstArgument(lp2, lp1, lp2)) {
				StrCpyN(myMsg, lp1, COUNTOF(myMsg));
				WideCharToMultiByte(CP_ACP, 0, myMsg, -1, fixconfont, 32, NULL, NULL);
			}
...
#ifdef BUILD_TE
        // TU: Full encoding support
        case L'U':
            if (lstrcmpi(lp1, L"UTF8") == 0) {
                fixmode = CPI_UTF8;
                flagSetEncoding = IDM_ENCODING_UTF8 - IDM_ENCODING_ANSI + 1;
            };
            if (lstrcmpi(lp1, L"UTF8S") == 0) {
                fixmode = CPI_UTF8SIGN;
                flagSetEncoding = IDM_ENCODING_UTF8SIGN - IDM_ENCODING_ANSI + 1;
            };
            if (lstrcmpi(lp1, L"U16") == 0) {
                fixmode = CPI_UNICODE;
                flagSetEncoding = IDM_ENCODING_UNICODE - IDM_ENCODING_ANSI + 1;
            };
            if (lstrcmpi(lp1, L"U16BOM") == 0) {
                fixmode = CPI_UNICODEBOM;
                flagSetEncoding = IDM_ENCODING_UTF8 - IDM_ENCODING_ANSI + 1;
            };
            break;

        case L'V':
            // /V or /VIEW - Start in Markdown Preview mode
            bStartInPreviewMode = TRUE;
            break;
#endif
		...
		case L'B':
			if (par2 == L'-') 	flagOldOn = TRUE;
            if (par2 == L'=') {
                int ili, illauf;
                ili = 0;
                illauf = 2;

                for (; lp1[illauf] != '\0' && ili < 60; illauf++) {
                    if (lp1[illauf] == '_') mymentxt[ili] = ' ';
                    else  mymentxt[ili]  = lp1[illauf];
                    ili++;
                    };
            ...
            if (par2 == L':') {  // Temp-file
                ...
                for (; lp1[illauf] != '\0' && ili < 60; illauf++) {
                    if (lp1[illauf] == '_') mytempo[ili] = ' ';
                    else  mytempo[ili] = lp1[illauf];
                    ili++;
                };
                mytempo[ili++] = '\0';
                mittemp = TRUE;
		...
		case L'Y':
		{
			BOOL bWindowProcessed = FALSE;

			if ((par2 == L'f') || (par2 == L'F')) {
				hparwindow = GetForegroundWindow();
			} else if ((par2 == L'w') || (par2 == L'W')) {
				hparwindow = (HWND)(INT_PTR)_wtoi(lp1+2);
			} else {
				hparwindow = NULL;
			}

			if (hparwindow && IsWindow(hparwindow)) {
				if (myconmod > 0) {
					RealGetWindowClass(hparwindow, myMsg, 63);
					if ((StrCmpN(myMsg, L"ConsoleW", 8)) == 0) {
						DWORD prId = 0;
						GetWindowThreadProcessId(hparwindow, &prId);
						AttachConsole(prId);
						HANDLE hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
						CONSOLE_FONT_INFOEX GETFONTEX;
						GETFONTEX.cbSize = sizeof(CONSOLE_FONT_INFOEX);
						if (GetCurrentConsoleFontEx(hStdOut, FALSE, &GETFONTEX) == 0) {
							ErrorMeld(L"GetConFont ge(h)t nicht", L" ");
						}
						int sogro = GETFONTEX.dwFontSize.Y;
						if (sogro > 0) {
							fixconsize = sogro;
							if (fixconsize < 200) fixconsize = fixconsize * myconmod;
						}
						WideCharToMultiByte(CP_ACP, 0, GETFONTEX.FaceName, -1, fixconfont, 32, NULL, NULL);
						FreeConsole();
					}
				}
				RECT rcOwner;
				...
				if (par0 == L'Y') {
					yoben = yoben + 2 * GetSystemMetrics(SM_CYCAPTION);
					xoben = xoben + GetSystemMetrics(SM_CYCAPTION);
				}
				bWindowProcessed = TRUE;
			}
		...
		case L'G':
		{
			LPWSTR pVal = lp1 + 1;  // nach dem 'G'
			if (*pVal == L':') pVal++;  // ':' überspringen falls vorhanden

			// Prüfen ob direkt Ziffern folgen (zB /G17 oder /G:17)
			if (*pVal >= L'0' && *pVal <= L'9') {
				int itok = swscanf(pVal, L"%i,%i", &iInitialLine, &iInitialColumn);
				if (itok == 1 || itok == 2) {
					flagJumpTo = 1;
				}
			}
			// Sonst nächstes Argument verwenden (zB /G 17)
			else if (ExtractFirstArgument(lp2, lp1, lp2)) {
				int itok = swscanf(lp1, L"%i,%i", &iInitialLine, &iInitialColumn);
				if (itok == 1 || itok == 2) {
					flagJumpTo = 1;
				}
			}
		...
		case L'M':
		{
					 ...
					 if (StrChr(lp1, L'R'))
						 bRegex = TRUE;
					 if (StrChr(lp1, L'S'))   // ms Spez. Search nach DF Nummer
						 bDFlex = TRUE;
					 if (StrChr(lp1, L'B'))
						 bTransBS = TRUE;
		...
		case L'Q':
			if ((par2 == L'C') || (par2 == L'c')) {
				myconmod = 64;
				break;
			};
			if ((par2 == L'D') || (par2 == L'd')) {
				myconmod = 100;
				break;
			};
			if ((par2 == L'W') || (par2 == L'w')) {
				myconmod = 50;
				break;
			};
		...
		case L'W':
		{
			LPWSTR pVal = lp1 + 1;  // nach dem 'W'
			if (*pVal == L':') pVal++;  // ':' überspringen falls vorhanden

			// Prüfen ob direkt Ziffern folgen (zB /W12345 oder /W:12345)
			if (*pVal >= L'0' && *pVal <= L'9') {
				hparwindow = (HWND)(ULONG_PTR)_wtoi64(pVal);
			}
			// Sonst nächstes Argument verwenden (zB /W 12345)
			else if (ExtractFirstArgument(lp2, lp1, lp2)) {
				hparwindow = (HWND)(ULONG_PTR)_wtoi64(lp1);
			}
		}
			break;

		case L'X':
 		    exit(myRunp(lp2, par2));
			break;
  ...
  if (lpFileArg == NULL) {
	  lpFileArg = GlobalAlloc(GPTR, sizeof(WCHAR)*(100 + 2)); // changed for ActivatePrevInst() needs
	  StrCpy(lpFileArg, L"neu.txt");
  };
...
/*  if (flagMultiFileArg == 0) {
  ...
  // Toolbar removed - flagToolbarLook setting removed
...
{
  WCHAR tch[MAX_PATH];
  if (GetPrivateProfileString(L"TE", L"N1.ini", L"", tch, COUNTOF(tch), lpszFile)) {
...
int TestIniFile() { 
  ...
  if (PathIsDirectory(szIniFile) || *CharPrev(szIniFile,StrEnd(szIniFile)) == L'\\') {
    ...
    if (!PathFileExists(szIniFile)) {
      lstrcpy(PathFindFileName(szIniFile),L"N1.ini");
      if (!PathFileExists(szIniFile)) {
  ...
  if (*lpszIniFile) {
    ...
    if (hFile != INVALID_HANDLE_VALUE) {
      if (GetFileSize(hFile,NULL) == 0) {
        DWORD dw;
        WCHAR tchExeName[MAX_PATH];
        WCHAR tchHeader[MAX_PATH];
        GetModuleFileName(NULL, tchExeName, COUNTOF(tchExeName));
        lstrcpy(tchExeName, PathFindFileName(tchExeName));
        PathRemoveExtension(tchExeName);
        wsprintf(tchHeader, L"\xFEFF[%s]\r\n", tchExeName);
        WriteFile(hFile,(LPCVOID)tchHeader,lstrlen(tchHeader)*sizeof(WCHAR),&dw,NULL);
      }
      CloseHandle(hFile);
      return(1);
    }
...
//=============================================================================
//
//  HighlightMatchingWords() - Highlight all occurrences of selected word
//
//
void HighlightMatchingWords(HWND hwnd)
{
  static char szLastWord[256] = "";
  char szWord[256] = "";
  int iSelStart, iSelEnd, iWordLen;
  int iDocLen, iPos, iTargetStart, iTargetEnd;

  if (!bHighlightMatchingWords) {
    return;
  }

  // Clear previous highlights
  SendMessage(hwnd, SCI_SETINDICATORCURRENT, INDIC_MATCHWORD, 0);
  ...
  iWordLen = iSelEnd - iSelStart;

  // Only highlight if selection is a word (1-255 chars, no spaces/newlines)
  if (iWordLen < 1 || iWordLen > 255) {
    szLastWord[0] = '\0';
    return;
  }
  ...
  // Check if it's a valid word (no whitespace)
  for (int i = 0; i < iWordLen; i++) {
    if (szWord[i] == ' ' || szWord[i] == '\t' || szWord[i] == '\r' || szWord[i] == '\n') {
      szLastWord[0] = '\0';
      return;
    }
  }
  ...
  if (strcmp(szWord, szLastWord) == 0) {
    return;
  }
  strcpy(szLastWord, szWord);

  // Setup indicator style (light yellow background)
  SendMessage(hwnd, SCI_INDICSETSTYLE, INDIC_MATCHWORD, INDIC_ROUNDBOX);
  SendMessage(hwnd, SCI_INDICSETFORE, INDIC_MATCHWORD, RGB(0, 180, 0));  // green outline
  SendMessage(hwnd, SCI_INDICSETALPHA, INDIC_MATCHWORD, 60);
  SendMessage(hwnd, SCI_INDICSETOUTLINEALPHA, INDIC_MATCHWORD, 180);
  SendMessage(hwnd, SCI_INDICSETUNDER, INDIC_MATCHWORD, TRUE);
  ...
  SendMessage(hwnd, SCI_SETTARGETEND, iDocLen, 0);
  SendMessage(hwnd, SCI_SETSEARCHFLAGS, SCFIND_WHOLEWORD | SCFIND_MATCHCASE, 0);
  ...
  while (iPos < iDocLen) {
    ...
    iTargetStart = (int)SendMessage(hwnd, SCI_SEARCHINTARGET, iWordLen, (LPARAM)szWord);
    if (iTargetStart < 0) {
      break;
    }

    iTargetEnd = (int)SendMessage(hwnd, SCI_GETTARGETEND, 0, 0);

    // Don't highlight the current selection itself
    if (iTargetStart != iSelStart) {
      SendMessage(hwnd, SCI_INDICATORFILLRANGE, iTargetStart, iTargetEnd - iTargetStart);
    }
...
void UpdateStatusbar()
{
...
#ifdef BUILD_TE
  // Show different status in preview mode
  if (bPreviewMode) {
    if (IsRtfPreviewMode()) {
      StatusSetText(hwndStatus, STATUS_DOCPOS, L"RTF Preview");
    } else if (IsHtmlPreviewMode()) {
      StatusSetText(hwndStatus, STATUS_DOCPOS, L"HTML Preview");
    } else {
      StatusSetText(hwndStatus, STATUS_DOCPOS, L"Markdown Preview");
    }
    StatusSetText(hwndStatus, STATUS_DOCSIZE, L"");
    StatusSetText(hwndStatus, STATUS_CODEPAGE, L"");
    StatusSetText(hwndStatus, STATUS_EOLMODE, L"");
    StatusSetText(hwndStatus, STATUS_OVRMODE, L"");
    StatusSetText(hwndStatus, STATUS_LEXER, L"Esc=Exit");
    return;
  }
#endif
  ...
  lstrcpy(tchLexerName, mEncoding[iEncoding].wchLabel);
  wsprintf(tchSel, L" %i", mEncoding[iEncoding].uCodePage);
//  lstrcat(tchLexerName, L" ");
  lstrcat(tchLexerName, tchSel);
  StatusSetText(hwndStatus,STATUS_CODEPAGE, tchLexerName);
//  StatusSetText(hwndStatus, STATUS_CODEPAGE, L"Hello...");
            ...
            BOOL *pbCancelDataLoss,BOOL bSaveCopy)
{
 ...
 // BeginWaitCursor();
  ...
  if (fLoad) {
//	 fSuccess = EditLoadFile(hwndEdit, psz, FALSE, ienc, ieol, pbUnicodeErr, pbFileTooBig);
   fSuccess = EditLoadFile(hwndEdit, psz, TRUE, ienc, ieol, pbUnicodeErr, pbFileTooBig);
   if (fSuccess) {
...
//		hFile = CreateFile(szFileName,  --fn)
	...
	//  if (iOriginalEncoding == 6)  iEncoding = 6;
  // fSuccess = FileIO(TRUE, szFileName, bNoEncDetect, &iEncoding, &iEOLMode, &bUnicodeErr, &bFileTooBig, NULL, FALSE);
  fSuccess = FileIO(TRUE,szFileName,1,&iEncoding,&iEOLMode,&bUnicodeErr,&bFileTooBig,NULL,FALSE);
   // if ( iOriginalEncoding == 6 )  iEncoding = 6;
  ...
  if (fSuccess) {
    ...
    bSaveWithBOM = (iEncoding == CPI_UTF8SIGN);
    bModified = FALSE;
    //bReadOnly = FALSE;
    SendMessage(hwndEdit,SCI_SETEOLMODE,iEOLMode,0);
...
BOOL FileSave(BOOL bSaveAlways,BOOL bAsk,BOOL bSaveAs,BOOL bSaveCopy)
{
  ...
  if (bnomenu) bAsk = FALSE;
  if (bnoAsk)  bAsk = FALSE;
...
//=============================================================================
//
//  OpenFileDlgWithPattern() - Open dialog with wildcard pre-filled
//
//
BOOL OpenFileDlgWithPattern(HWND hwnd,LPWSTR lpstrFile,int cchFile,LPCWSTR lpstrInitialDir,LPCWSTR lpstrPattern)
{
  OPENFILENAME ofn;
  WCHAR szFile[MAX_PATH];
  WCHAR szFilter[NUMLEXERS*1024];

  lstrcpyn(szFile, lpstrPattern ? lpstrPattern : L"", COUNTOF(szFile));
  Style_GetOpenDlgFilterStr(szFilter,COUNTOF(szFilter));
...
//=============================================================================
//
//  SaveFileDlgHook() - Hook procedure for Save dialog BOM checkbox
//
//
UINT_PTR CALLBACK SaveFileDlgHook(HWND hdlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  (void)wParam;
  switch (uMsg) {
    case WM_INITDIALOG:
      // Only show BOM checkbox for UTF-8 editors (TU), hide for ANSI/OEM (TX/TY)
      if (iDefaultEncoding == CPI_UTF8 || iDefaultEncoding == CPI_UTF8SIGN) {
        CheckDlgButton(hdlg, IDC_SAVEWITHBOM, bSaveWithBOM ? BST_CHECKED : BST_UNCHECKED);
      } else {
        ShowWindow(GetDlgItem(hdlg, IDC_SAVEWITHBOM), SW_HIDE);
      }
      return TRUE;
    ...
    case WM_NOTIFY:
      {
        ...
        if (pofn->hdr.code == CDN_FILEOK) {
          // User clicked Save - update bSaveWithBOM from checkbox state
          bSaveWithBOM = (IsDlgButtonChecked(hdlg, IDC_SAVEWITHBOM) == BST_CHECKED);
        }
      }
      break;
  }
...
BOOL SaveFileDlg(HWND hwnd,LPWSTR lpstrFile,int cchFile,LPCWSTR lpstrInitialDir)
{
  ...
  ofn.lpstrDefExt = (lstrlen(tchDefaultExtension)) ? tchDefaultExtension : NULL;
  ofn.lpfnHook = SaveFileDlgHook;
  ofn.lpTemplateName = MAKEINTRESOURCE(IDD_SAVEBOM);
...
* ActivatePrevInst()
*
* Tries to find and activate an already open TU Window
*
*
******************************************************************************/
...
//  SnapToDefaultPos()
//
//  Aligns TU to the default window position on the current screen
//
//
