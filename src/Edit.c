...
* TU - Text Editor
*
* Edit.c - Text File Editing Helper Stuff
*
...
//=============================================================================
//
//  Encoding Helper Functions !fn
//
void Encoding_InitDefaults() {
  wsprintf(wchANSI,L" (%i)",GetACP());
  mEncoding[CPI_OEM].uCodePage = GetOEMCP();
  if (fixenc > 0) mEncoding[CPI_OEM].uCodePage = fixenc;
...
typedef struct _np2encoding {
  ...
  */
  ...
  iDefaultEncoding = fixmode; // CPI_UTF8;
  ...
  *iEncoding = iDefaultEncoding;
  ...
  if (fixenc > 0) {
      mEncoding[*iEncoding].uCodePage = fixenc;
  };
  ...
  if (cbData == 0) {   // leer
    ...
    // if (!bSkipEncodingDetection && (iSrcEncoding == -1 || iSrcEncoding == CPI_UNICODE || iSrcEncoding == CPI_UNICODEBE) &&
...
#define IsUTF16BOM(p) \
  ...
  if ((fixmode == CPI_UTF8) || (fixmode == CPI_UTF8SIGN)) { 
      if (!IsUTF16BOM(lpData)) {
          goto mitwinutf8;
      };
 //     if (!bBOM) goto mitwinutf8;        // mode zwar UTF8 aber mit BOM C
  };

  if (IsUTF8Signature(lpData)) goto mitwinutf8;
       ...
       //   else if (iSrcEncoding == CPI_UNICODEBE)
        ...
        //  else { */
              if (bBOM) *iEncoding = CPI_UNICODEBOM;
              else      *iEncoding = CPI_UNICODE;
      //    }
 ...
 mitwinutf8:
       ...
       //   if ((iSrcEncoding == CPI_UTF8 || iSrcEncoding == CPI_UTF8SIGN) &&
...
/*              if (iSrcEncoding != -1)
          ...
          //    int neui1 = mEncoding[*iEncoding].uFlags;
          //    int neui2 = mEncoding[*iEncoding].uCodePage;
       ...
       BOOL bSaveCopy)
{
  ...
  *pbCancelDataLoss = FALSE;
  ...
  // Check if UTF-16 mode (via /U16 or /U16BOM command line)
  if (iEncoding == CPI_UNICODE || iEncoding == CPI_UNICODEBOM) {
      LPWSTR lpDataWide;
      int    cbDataWide;
  ...
  // Default: Save as UTF-8 (with or without BOM)
  SetEndOfFile(hFile);

  // Write BOM if bSaveWithBOM is set (from checkbox or file was opened with BOM)
  if (bSaveWithBOM)
...
//
//  EditBeautifyCPS()
//  For .cps files: replace bare ">" lines with "> " for consistent syntax highlighting
//
void EditBeautifyCPS(HWND hwnd)
{
  int line;
  int maxLines;
  int lineStart;
  ...
  for (line = 0; line < maxLines; line++)
  {
    ...
    int pos = lineStart;
    while (pos < lineEnd && ((char)SendMessage(hwnd, SCI_GETCHARAT, pos, 0) == ' ' ||
                              (char)SendMessage(hwnd, SCI_GETCHARAT, pos, 0) == '\t'))
      pos++;

    // Check if line is just ">" (nothing after it)
    if (pos < lineEnd &&
        (char)SendMessage(hwnd, SCI_GETCHARAT, pos, 0) == '>' &&
        pos + 1 == lineEnd)
    {
      // Append a space after >
      SendMessage(hwnd, SCI_SETTARGETSTART, lineEnd, 0);
      SendMessage(hwnd, SCI_SETTARGETEND, lineEnd, 0);
      SendMessage(hwnd, SCI_REPLACETARGET, 1, (LPARAM)" ");
    }
  }
  SendMessage(hwnd, SCI_ENDUNDOACTION, 0, 0);
}
    ...
    case WM_MOUSEMOVE:
      {
        ...
        int nId = (int)GetWindowLong(hwndHover,GWL_ID);

        if (GetActiveWindow() == hwnd) {
          if (nId >= 200 && nId <= 205) {
            if (id_capture == nId || id_capture == 0) {
...
BOOL FileVars_Init(char *lpData,DWORD cbData,LPFILEVARS lpfv) {
  ...
  fNoFileVariables = TRUE; // ???
  bNoEncodingTags = TRUE;
