...
* TU - Text Editor
*
* Helpers.c - General helper functions
...
// Toolbar removed - Toolbar_Get/SetButtons() functions removed
/*
//=============================================================================
//
...
void PathAbsoluteFromApp(LPWSTR lpszSrc,LPWSTR lpszDest,int cchDest,BOOL bExpandEnv) {
  ...
  if (StrCmpNI(lpszSrc, L"%CSIDL:MYDOCUMENTS%", CSTRLEN("%CSIDL:MYDOCUMENTS%")) == 0) {
	  ...
	  if (lpszSrc != NULL) lstrcpyn(wchPath, lpszSrc, COUNTOF(wchPath));
	  else lstrcpyn(wchPath, L"", COUNTOF(wchPath));
...
//
//  Purpose: Modified to create a desktop link to TU
//
//  Manipulates:
//
...
//
//  Purpose: Modified to create a TU favorites link
//
//  Manipulates:
//
...
BOOL GetThemedDialogFont(LPWSTR lpFaceName,WORD* wSize)
{
  ...
  if ((hModUx = GetModuleHandle(L"uxtheme.dll")) != NULL) {
    if ((BOOL)(GetProcAddress(hModUx,"IsAppThemed"))()) {
      hTheme = (HTHEME)(INT_PTR)(GetProcAddress(hModUx,"OpenThemeData"))(NULL,L"WINDOWSTYLE;WINDOW");
      if (hTheme) {
        if (S_OK == (HRESULT)(GetProcAddress(hModUx,"GetThemeSysFont"))(hTheme,/*TMT_MSGBOXFONT*/805,&lf)) {
          if (lf.lfHeight < 0)
        ...
        (GetProcAddress(hModUx,"CloseThemeData"))(hTheme);
      }
    }
  }
